...
function renderValidationSummary(state) {
  const count = state.computed.errors.length;
  const details = el('details', { cls: `sheet-status no-print${state.computed.valid ? ' ok' : ''}` });
  details.append(el('summary', {
    text: state.computed.valid
      ? 'Build checks out for play.'
      : `Play sheet is still showing ${count} build issue${count === 1 ? '' : 's'}.`
  }));

  if (state.computed.valid) {
    details.append(el('p', { cls: 'muted', text: 'Everything required for a legal Greenhorn build is passing.' }));
  } else {
    details.append(el('ul', { cls: 'summary-list' }, state.computed.errors.map((error) => el('li', { text: error }))));
  }
...
export function renderSheetStep(state, mutateCharacter) {
  ...
  root.append(renderValidationSummary(state));
  ...
  sheet.append(renderPortraitPanel(state, mutateCharacter));
