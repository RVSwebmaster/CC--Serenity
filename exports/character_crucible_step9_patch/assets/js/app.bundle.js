(() => {
  ...
  function normalizePurchasedGearEntry(entry = {}) {
    ...
    return {
      id: entry.id || makeId2(),
      catalogId: entry.catalogId || (catalogMatch == null ? void 0 : catalogMatch.id) || "",
      name: entry.name || (catalogMatch == null ? void 0 : catalogMatch.name) || "",
      category: entry.category || (catalogMatch == null ? void 0 : catalogMatch.category) || "",
      credits: Number.parseFloat((_b = (_a = entry.credits) != null ? _a : catalogMatch == null ? void 0 : catalogMatch.credits) != null ? _b : 0) || 0,
      availability: entry.availability || (catalogMatch == null ? void 0 : catalogMatch.availability) || "",
      note: entry.note || (catalogMatch == null ? void 0 : catalogMatch.summary) || ""
    };
  }
  ...
  function hydrateCharacter(input) {
    ...
    if (!character.basics.portraitDataUrl) {
      const legacyPortrait = (_c = (_a = input.basics) == null ? void 0 : _a.portraitUrl) != null ? _c : (_b = input.details) == null ? void 0 : _b.portraitUrl;
    ...
    character.details.purchasedGear = (((_d = input.details) == null ? void 0 : _d.purchasedGear) || []).map((item) => normalizePurchasedGearEntry(item));
    character.traits.assets = (((_e = input.traits) == null ? void 0 : _e.assets) || character.traits.assets).map((trait) => ({
    ...
    character.traits.complications = (((_f = input.traits) == null ? void 0 : _f.complications) || character.traits.complications).map((trait) => ({
      id: trait.id || (globalThis.crypto && typeof globalThis.crypto.randomUUID === "function" ? globalThis.crypto.randomUUID() : createEmptySpecialty().id),
    ...
    SKILLS.forEach((skill) => {
      var _a2;
      const incoming = (_a2 = input.skills) == null ? void 0 : _a2[skill.name];
      character.skills[skill.name] = {
        generalRating: (incoming == null ? void 0 : incoming.generalRating) || "none",
        specialties: ((incoming == null ? void 0 : incoming.specialties) || []).map((specialty) => ({
  ...
  function resolveRoleLabel(basics) {
    ...
    if ((basics.role || "") === "Other") {
      return ((_a = basics.customRole) == null ? void 0 : _a.trim()) || "Other";
    }
    return basics.role || "\u2014";
  }
  ...
  function dieCost(die) {
    var _a;
    return (_a = DIE_COSTS[die]) != null ? _a : 0;
  }
  function specialtyDieCost(die) {
    var _a;
    return (_a = SPECIALTY_DIE_COSTS[die]) != null ? _a : 0;
  }
  ...
  function effectiveGeneralRating(character, skillName) {
    var _a, _b, _c;
    const purchased = ((_b = (_a = character.skills) == null ? void 0 : _a[skillName]) == null ? void 0 : _b.generalRating) || "none";
    if ((((_c = character.basics) == null ? void 0 : _c.roleSkill) || "") === skillName) {
  ...
  function renderWelcomeStep(state2, onChange) {
    ...
    quote.value = state2.character.basics.quote || "";
    quote.placeholder = "Optional one-line quote";
    quote.addEventListener("input", (event) => onChange((draft) => {
      draft.basics.quote = event.target.value;
    }));
    left.append(field("Quote", quote, "Optional. A line worth seeing beneath their name on the finished sheet."));
    right.append(el("div", { cls: "summary-card guide-card" }, [
      ...
      el("p", { cls: "muted", text: "Name, concept, and an optional quote all carry forward to the finished sheet, so keep them concise and specific." })
    ]));
  ...
  function chooseActiveTrait(list, category) {
    ...
    activeTraitSelection[category] = (active == null ? void 0 : active.id) || null;
    return active;
  }
  ...
  function buildTraitSection(title, list, oppositeList, curated, onChange, category) {
    ...
    wrap.append(infoBox);
    const setInfo = (trait) => {
      if (trait == null ? void 0 : trait.id) activeTraitSelection[category] = trait.id;
    ...
    list.forEach((trait) => {
      ...
      const allowedRatings = (traitMeta == null ? void 0 : traitMeta.allowedRatings) || TRAIT_OPTIONS.filter((item) => item !== "none");
      ...
      removeButton.addEventListener("click", () => onChange((draft) => {
        ...
        if (activeTraitSelection[category] === trait.id) activeTraitSelection[category] = ((_a = draft.traits[key][0]) == null ? void 0 : _a.id) || null;
      }));
  ...
  function renderSkillsStep(state2, onChange) {
    ...
    if (state2.character.basics.roleSkill) {
      const purchased = ((_a = state2.character.skills[state2.character.basics.roleSkill]) == null ? void 0 : _a.generalRating) || "none";
  ...
  function getDefaultStartingCredits(character = {}) {
    var _a, _b;
    if (hasSelectedTrait((_a = character.traits) == null ? void 0 : _a.assets, "Moneyed Individual")) return 1125;
    if (hasSelectedTrait((_b = character.traits) == null ? void 0 : _b.complications, "Dead Broke")) return 375;
    return 750;
  }
  function resolveStartingCredits(character = {}) {
    var _a;
    const explicit = (_a = character.details) == null ? void 0 : _a.startingCredits;
  ...
  function resolveDamageMax(state2) {
    ...
    const currentStun = Number.parseInt((_b = (_a = state2.character.trackers) == null ? void 0 : _a.stun) != null ? _b : 0, 10);
    const currentWounds = Number.parseInt((_d = (_c = state2.character.trackers) == null ? void 0 : _c.wounds) != null ? _d : 0, 10);
  ...
  function renderDamageDrawer(state2, mutateCharacter2) {
    ...
    const stun = clampTrackerValue((_b = (_a = state2.character.trackers) == null ? void 0 : _a.stun) != null ? _b : 0, max);
    const wounds = clampTrackerValue((_d = (_c = state2.character.trackers) == null ? void 0 : _c.wounds) != null ? _d : 0, max);
  ...
  function renderPlotPointTracker(character, mutateCharacter2) {
    var _a, _b;
    const current = clampTrackerValue((_b = (_a = character.trackers) == null ? void 0 : _a.plotPoints) != null ? _b : 1, PLOT_POINT_MAX);
    const clear = el("button", {
      cls: "damage-clear-button",
      text: "Clear",
  ...
  function renderValidationSummary(state2) {
    const count = state2.computed.errors.length;
    const details = el("details", { cls: `sheet-status no-print${state2.computed.valid ? " ok" : ""}` });
    details.append(el("summary", {
      text: state2.computed.valid ? "Build checks out for play." : `Play sheet is still showing ${count} build issue${count === 1 ? "" : "s"}.`
    }));
    if (state2.computed.valid) {
      details.append(el("p", { cls: "muted", text: "Everything required for a legal Greenhorn build is passing." }));
    } else {
      details.append(el("ul", { cls: "summary-list" }, state2.computed.errors.map((error) => el("li", { text: error }))));
    }
    return details;
  }
  function renderSheetStep(state2, mutateCharacter2) {
    const root = el("div");
    root.append(el("div", { cls: "no-print" }, [
      sectionHeader("Step 9: Play Sheet", "This is the ready-to-play handoff. Keep it open beside the table, print it, or export it when the build is set.")
    ]));
    root.append(renderValidationSummary(state2));
    const stage = el("div", { cls: "sheet-stage" });
    const sheet = el("div", { cls: "sheet-layout", attrs: { id: "printableSheet" } });
    sheet.append(el("section", { cls: "sheet-hero" }, [
      ...
      el("div", { cls: "sheet-block sheet-hero-meta" }, [
        el("div", { cls: "sheet-meta-list" }, [
    ...
    sheet.append(renderPortraitPanel(state2, mutateCharacter2));
  ...
  function restoreInputFocus(root, snapshot) {
    ...
    for (const index of snapshot.path) {
      node = (_a = node == null ? void 0 : node.children) == null ? void 0 : _a[index];
      if (!node) return;
    }
  ...
  function render(options = {}) {
    ...
    if (current.id === "review") {
      ...
      els.nextBtn.textContent = "Export to Amanuensis";
    } else {
    ...
    if (current.id === "sheet") {
      setMessage(els.messageBar, computed.valid ? "Play sheet is ready. Print it or export it to Amanuensis." : "Play sheet is open. A few build issues still need cleanup before export.", computed.valid ? "ok" : "warn");
